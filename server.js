 ...
 * Bluprint backend
 * ------------------------------------------------------------
 * A standalone Express API that:
 *   1. Fetches live job listings server-side from real company
 *      job-board APIs (Greenhouse + Lever) — no CORS issues,
 *      because this runs on a server, not in the browser.
 *   2. Caches those listings for SYNC_INTERVAL_MS so we don't
 *      hammer the company boards on every page load.
 *   3. Persists per-user profile / saved jobs / alerts /
 *      applications to a JSON file on disk (data.json).
 *
 * This is a lightweight, dependency-minimal backend meant to
 * replace the browser-side window.storage calls in the React
 * frontend. It is NOT production-hardened — see the notes at
 * the bottom of this file for what a real deployment needs.
 *
 * Run it:
 *   npm install
 *   node server.js
